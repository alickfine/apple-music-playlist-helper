...
public enum CLIOptionsError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        switch self {
        case .missingSubcommand: "缺少子命令；当前仅支持 add。"
...
public struct CLIOptions: Equatable, Sendable {
    ...
    public static func parse(_ arguments: [String]) throws -> CLIOptions {
        guard let command = arguments.first else { throw CLIOptionsError.missingSubcommand }
        guard command == "add" else { throw CLIOptionsError.unsupportedSubcommand(command) }
        var playlist: String?
        var input: URL?
        var create = false
        var dryRun = false
        ...
        while index < arguments.count {
            ...
            case "--create": create = true; index += 1
            case "--dry-run": dryRun = true; index += 1
            case "--play-first": playFirst = true; index += 1
        ...
        return CLIOptions(
            playlist: playlist, input: input, create: create, dryRun: dryRun,
